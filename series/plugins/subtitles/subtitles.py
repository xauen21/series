...
class Subtitles:
	...
	subtitles_path = pkg_resources.resource_filename("data", "subtitles")
	found = "Link found."
		...
		for plugin_module, plugin_class in Subtitles.getPlugins().items():
			...
			for name in names:
				for episode in serie.episodes:
					...
					link, code = Subtitles.searchUrl(plugin_module, plugin_class, name, episode)
	...
	def download(episode, plugin = None):
		for plugin_module, plugin_class in Subtitles.getPlugins(plugin).items():
			plugin_name = plugin_class.lower()
			...
			id = str(uuid.uuid4())
			working_dir = Subtitles.subtitles_path + "/" + id
			...
			for srt_file in os.listdir(working_dir):
				if ".srt" in srt_file:
					shutil.move(working_dir + "/" + srt_file, Subtitles.subtitles_path + "/" + Path(srt_file).name)
					subtitle_file = Subtitles.subtitles_path + "/" + Path(srt_file).name
					break
	...
	def getPlugins(plugin = None):
		...
		plugin_names = list(filter(lambda x: "__" not in x and "subtitles.py" not in x and (not plugin or plugin + ".py" == x), plugin_files))
